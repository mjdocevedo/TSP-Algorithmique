  ...
  # to build the search space tree recursively
    ...
    for (i in 1:N){
      ...
      if (adj[curr_path[level-1],i] != 0 & visited[i] == FALSE){
        ...
        if (level == 2)
          curr_bound = curr_bound - ((firstMin(adj, curr_path[level-1]) + firstMin(adj, i))/ 2)
        else
          curr_bound = curr_bound - ((secondMin(adj, curr_path[level-1]) + firstMin(adj, i))/2)
...
TSP <- function(adj){
  ...
  TSPRec(adj, curr_bound, 0, 2, curr_path, visited)
}
...
adj = matrix(c(0, 10, 15, 20,
               10, 0, 35, 25,
               15, 35, 0, 30,
               20, 25, 30, 0), nrow = 4, ncol = 4)
N = 4

# final_path[] stores the final solution
# i.e. the // path of the salesman.
final_path = c(rep(NULL,N+1))
